...
function renderMarkdown(content) {
  ...
  try {
    ...
    marked.setOptions({
      ...
      sanitize: false,    // 不过滤HTML（因为我们信任内容）
    ...
    // 如果渲染失败，返回原始内容
    return `<pre style="background:#f7f8fa;padding:12px 14px;border-radius:8px;white-space:pre-wrap;font-size:15px;line-height:1.7;">${content}</pre>`;
  }
}
...
window.showPromptDetail = function(idx) {
  ...
  // 渲染Markdown内容
  const markdownContent = renderMarkdown(p['提示词内容']?.trim() || '');
  ...
  modalBodyEl.innerHTML = `
    ...
    <div class="markdown-content" style="background:#f7f8fa;padding:12px 14px;border-radius:8px;font-size:15px;line-height:1.7;">${markdownContent}</div>
